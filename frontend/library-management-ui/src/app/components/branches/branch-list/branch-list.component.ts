...
import { FormsModule } from '@angular/forms'; // 1. Added FormsModule
...
import {
  ...
  UpdateBranchRequest,
} from '../../../models/branch.models'; // Added BranchSearchResponse & UpdateBranchRequest
...
@Component({
  ...
  imports: [
    ...
    ToastComponent,
    SpinnerComponent,
  ], // Added FormsModule here
  templateUrl: './branch-list.component.html',
  styleUrl: './branch-list.component.scss',
})
export class BranchListComponent implements OnInit {
  // 2. Updated types to BranchSearchResponse
  branches: BranchSearchResponse[] = [];
  pagination: PaginatedResult<BranchSearchResponse> | null = null;
  loading = false;
  errorMessage = '';
  searchName = '';
  searchCode = '';
      ...
      .subscribe({
        next: (response) => {
          ...
          if (response.success && response.data) {
            ...
            this.totalPages = 0;
            this.errorMessage =
              response.errors?.join(' ') || 'Failed to load branches';
          }
        },
        error: (err) => {
          this.loading = false;
  ...
  onDelete(branch: BranchSearchResponse): void {
    if (
      !confirm(
    ...
    this.branchService.delete(branch.id).subscribe({
      next: (response) => {
        if (response.success) {
          ...
          this.errorMessage =
            response.errors?.join(' ') || 'Failed to delete branch';
          this.showToast('error', this.errorMessage);
        }
      },
      error: (err) => {
  ...
  onToggleActive(branch: BranchSearchResponse): void {
    // 3. Construct a clean UpdateBranchRequest payload
    const updatePayload: UpdateBranchRequest = {
      name: branch.name,
      code: branch.code,
      address: branch.address,
      phone: branch.phone ?? undefined, // Convert null to undefined
      email: branch.email ?? undefined, // Convert null to undefined
      isActive: !branch.isActive,
    };

    this.branchService.update(branch.id, updatePayload).subscribe({
      next: (response) => {
        if (response.success) {
          this.showToast(
            'success',
            `Branch "${branch.name}" ${branch.isActive ? 'deactivated' : 'activated'} successfully.`,
          );
          this.loadBranches();
        } else {
          this.showToast(
