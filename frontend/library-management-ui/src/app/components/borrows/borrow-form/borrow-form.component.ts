...
export class BorrowFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.isMember = this.authService.getRole() === 'Member';
    if (this.isMember) {
      this.form.memberId = this.getCurrentMemberId();
    }
  }

  private getCurrentMemberId(): string {
  ...
  onSubmit(): void {
    this.errors = {};
    if (!this.form.bookCopyId.trim())
      this.errors['bookCopyId'] = 'Book copy is required';
    if (!this.form.memberId.trim())
      this.errors['memberId'] = 'Member is required';
      ...
      .subscribe({
        next: (response) => {
          ...
          if (response.success && response.data) {
            this.showToast(
              'success',
              `Book "${response.data.bookTitle}" borrowed successfully.`,
            );
            this.router.navigate(['/borrows']);
          } else {
            this.errorMessage =
              response.errors?.join(' ') || 'Failed to create borrow';
